...
function renderActivePlayer(players) {
    ...
    // Если нет текущего игрока, показываем сообщение
    if (!currentPlayer) {
        container.innerHTML = `
...
async function arrangeTurnOrder() {
    try {
        const res = await fetch('/api/game/arrange_turns', {
            method: 'POST'
        });

        if (res.ok) {
            showNotification('Очередность ходов установлена!', 'success');
            await fetchPlayers();
            await fetchTeams();
        } else {
            throw new Error('Ошибка установки очередности');
        }
    } catch (error) {
        console.error('Ошибка установки очередности:', error);
        showNotification('Ошибка установки очередности!', 'error');
    }
}

async function skipPlayerTurn(playerId) {
    try {
        const res = await fetch(`/api/players/${playerId}/skip`, {
            method: 'PUT'
        });

        if (res.ok) {
            showNotification('Ход пропущен!', 'success');
            await fetchPlayers();
            await fetchTeams();
        } else {
            throw new Error('Ошибка пропуска хода');
        }
    } catch (error) {
        console.error('Ошибка пропуска хода:', error);
        showNotification('Ошибка пропуска хода!', 'error');
    }
}

async function setPlayerTurnOrder(playersOrder) {
    try {
        const res = await fetch('/api/players/turn-order', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(playersOrder)
        });

        if (res.ok) {
            showNotification('Порядок ходов обновлен!', 'success');
            await fetchPlayers();
        } else {
        ...
        showNotification('Ошибка обновления порядка!', 'error');
    }
}

// Функция для отображения управления очередностью
// Функция для отображения управления очередностью
function renderTurnOrderManagement() {
    const container = document.getElementById('turn-order-container');
    if (!container) return;

    const team1Players = players.filter(p => p.team_id === 1).sort((a, b) => (a.turn_order || 999) - (b.turn_order || 999));
    const team2Players = players.filter(p => p.team_id === 2).sort((a, b) => (a.turn_order || 999) - (b.turn_order || 999));

    const currentPlayer = players.find(p => p.is_current);

    container.innerHTML = `
        <div class="turn-order-section">
            <h3>🎮 Управление очередностью ходов</h3>
            
            <div class="turn-order-controls">
                <button class="magic-btn" onclick="arrangeTurnOrder()" ${players.length < 2 ? 'disabled' : ''}>
                    🔄 Авторасстановка
                </button>
                <button class="magic-btn" onclick="showManualOrderModal()" ${players.length === 0 ? 'disabled' : ''}>
                    ✏️ Ручная настройка
                </button>
            </div>
            ...
            ${currentPlayer ? `
                <div class="current-turn-info">
                    ...
                    <div class="current-player-display">
                        <span class="player-name ${currentPlayer.has_moved ? 'player-name-moved' : ''}">
                            ${currentPlayer.name}
                        </span>
                        <span class="player-team">(Команда ${currentPlayer.team_id})</span>
                        <button class="skip-turn-btn" onclick="skipPlayerTurn(${currentPlayer.id})">
                            ⏭️ Пропустить ход
                        </button>
                    </div>
                </div>
            ` : `
                <div class="current-turn-info">
                    <h4>Текущий ход:</h4>
                    <div class="current-player-display">
                        <span style="color: #ff6b6b;">❌ Нет текущего игрока</span>
                        <button class="skip-turn-btn" onclick="arrangeTurnOrder()" style="background: #4caf50;">
            ...
            <div class="turn-order-visualization">
                <div class="team-turn-order">
                    <h4>Команда 1:</h4>
                    ${team1Players.length > 0 ? team1Players.map(player => `
                        <div class="turn-order-item ${player.is_current ? 'current-turn' : ''} ${player.has_moved ? 'moved-turn' : ''}">
                            <span class="turn-order-number">${player.turn_order || '?'}</span>
                            <span class="player-name">${player.name}</span>
                            ${player.is_current ? '<span class="current-indicator">▶️</span>' : ''}
                        </div>
                    `).join('') : '<div style="padding: 10px; color: #666;">Игроков нет</div>'}
                </div>
...
// Модальное окно для ручной настройки порядка
function showManualOrderModal() {
    if (players.length === 0) {
        showNotification('Нет игроков для настройки порядка!', 'error');
        return;
    }

    const modal = document.createElement('div');
    modal.id = 'turn-order-modal';
    modal.className = 'edit-event-modal';
    modal.style.cssText = `
        position: fixed;
        top: 0;
    ...
    const sortedPlayers = [...players].sort((a, b) => (a.turn_order || 999) - (b.turn_order || 999));
    ...
    modal.innerHTML = `
        ...
        ">
            <div style="position: absolute; top: 5px; left: 50%; transform: translateX(-50%); 
                       background: #1976d2; color: #ffd700; padding: 10px 20px; 
                       border-radius: 20px; border: 2px solid #ffd700; font-weight: bold;">
                Настройка порядка ходов
            </div>
            
            <div style="margin: 40px 0 20px 0;">
                <p style="color: #5D4037; margin-bottom: 15px;">
                    Перетащите игроков для установки порядка ходов (чередование команд):
                </p>
                
                <div id="turn-order-list" class="turn-order-list">
                    ${sortedPlayers.map((player, index) => `
                        <div class="draggable-player" data-player-id="${player.id}" draggable="true">
                            <span class="drag-handle">☰</span>
                            <span class="player-order-number">${index + 1}</span>
                            <span class="player-name">${player.name}</span>
                            <span class="player-team">(Команда ${player.team_id})</span>
                        </div>
                    `).join('')}
                </div>
            </div>
            ...
            <div style="display: flex; gap: 15px; justify-content: center; margin-top: 20px;">
                <button id="order-cancel-btn" class="magic-btn" 
                        style="background: linear-gradient(135deg, #8B4513, #a0522d);">
                    Отмена
                </button>
                <button id="order-save-btn" class="magic-btn" 
                        style="background: linear-gradient(135deg, #1976d2, #1565c0);">
                    Сохранить порядок
                </button>
            </div>
    ...
    document.getElementById('order-save-btn').addEventListener('click', saveTurnOrder);
...
function setupDragAndDrop() {
    ...
    list.querySelectorAll('.draggable-player').forEach(item => {
        ...
        item.addEventListener('drop', function(e) {
            ...
            if (draggedItem && this !== draggedItem) {
                const allItems = Array.from(list.querySelectorAll('.draggable-player'));
...
function updateOrderNumbers() {
    const items = document.querySelectorAll('.draggable-player');
    items.forEach((item, index) => {
        item.querySelector('.player-order-number').textContent = index + 1;
    });
}

function saveTurnOrder() {
    const items = document.querySelectorAll('.draggable-player');
    const playersOrder = Array.from(items).map((item, index) => ({
        id: parseInt(item.dataset.playerId),
        turn_order: index + 1
    }));

    setPlayerTurnOrder(playersOrder);
    closeOrderModal();
}
